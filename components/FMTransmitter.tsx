...
import { Radio, Music, Volume2, FastForward, Rewind, Play, Pause, Bluetooth, Zap, SignalHigh, Globe, Waves, Activity } from 'lucide-react';
      ...
      <div className="bg-slate-950 rounded-[2rem] p-8 text-center border-2 border-slate-800 relative overflow-hidden shadow-2xl group ring-1 ring-white/5">
        ...
        <div className="flex justify-center gap-1.5 h-8 items-end mt-2">
          {[...Array(20)].map((_, i) => (
            <div 
              key={i} 
              className={`w-1 rounded-full transition-all duration-150 ${isTransmitting ? 'bg-gradient-to-t from-blue-600 to-blue-400' : 'bg-slate-800'}`}
              style={{ height: isTransmitting ? `${20 + Math.random() * 80}%` : '15%' }}
            ></div>
          ))}
        </div>
      ...
      <div className="bg-slate-950/60 rounded-3xl p-6 border border-slate-800 relative shadow-inner">
         <div className="flex justify-between items-center mb-6">
            <div className="flex flex-col items-center gap-2 group">
              <div className="w-10 h-10 rounded-xl bg-slate-900 flex items-center justify-center border border-slate-800 group-hover:border-blue-500/50 transition-colors">
            ...
            <div className="flex-1 flex flex-col items-center gap-2">
              ...
              <div className="flex gap-4">
                <Zap size={14} className={isTransmitting ? 'text-blue-500 animate-bounce' : 'text-slate-800'} />
